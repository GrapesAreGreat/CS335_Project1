...
sorted_disks sort_alternate(const disk_state& before) {
    disk_state slate = before;
    size_t totalnum = slate.total_count();
    int numOfSwap = 0;

    for (size_t i = 0; i < totalnum + 1 && !slate.is_sorted(); ++i) {
        for (size_t j = i % 2; j < totalnum - 1; j += 2) {
            if (slate.get(j) == DISK_DARK && slate.get(j + 1) == DISK_LIGHT) {
                slate.swap(j);
                numOfSwap++;
            }
        }
    ...
    return sorted_disks(disk_state(slate), numOfSwap);
}
